...
法官语音资产使用 `/api/v1/admin/judge-voice-lines*` 提供 `voice.read` 保护的覆盖率、分类筛选、缺失项和受认证试听；普通 DTO 不返回文件路径、旧 public URL、manifest、字幕内容或音频字节。当前只读页仍从旧静态目录读取，独立持久存储与异步生成任务留到后续写操作切片。旧 Web 匿名生成 POST 默认关闭，production 禁止重新开启。
