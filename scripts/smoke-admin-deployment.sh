...
wait_for_status "/healthz" "200" "$work_dir/admin-health.txt"
grep -q '^ok' "$work_dir/admin-health.txt"
...
wait_for_status "/api/v1/admin/me" "401" "$work_dir/admin-me.json"
wait_for_status "/api/v1/metrics" "404" "$work_dir/metrics.txt"

wait_for_status "/system/users" "200" "$work_dir/deep-link.html"
grep -q 'id="root"' "$work_dir/deep-link.html"

echo "admin deployment smoke checks passed: ${base_url}"
