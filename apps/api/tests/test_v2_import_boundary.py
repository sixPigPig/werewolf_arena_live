...
def test_api_v2_does_not_import_old_werewolf_business_modules() -> None:
    violations: list[str] = []
    for path in sorted((REPOSITORY_ROOT / "apps/api/app/match").rglob("*.py")):
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        for node in ast.walk(tree):
            forbidden = ("app.werewolf", "app.v2")
            if isinstance(node, ast.ImportFrom) and (node.module or "").startswith(forbidden):
                violations.append(f"{path.relative_to(REPOSITORY_ROOT)}:{node.lineno}:{node.module}")
...
def test_web_v2_does_not_import_old_game_live_or_replay_modules() -> None:
    ...
    violations: list[str] = []
    for directory in (
        REPOSITORY_ROOT / "apps/mobile-web/src/match",
        REPOSITORY_ROOT / "apps/admin-web/src/match",
    ):
