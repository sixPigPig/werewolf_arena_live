...
def test_reaper_fails_expired_ready_lease() -> None:
    ...
    reaped = repository.reap_stale_runs(stale_grace_seconds=30)
    assert [entry["game_id"] for entry in reaped] == [GAME_ID]
    ...
    with factory() as db:
        ...
        reaped_event = db.scalar(
            select(GameRecordEvent).where(
                GameRecordEvent.event_type == "v2_run_execution_reaped"
            )
        )
        assert reaped_event is not None
        assert reaped_event.payload["failure_code"] == "worker_lease_expired"
        assert reaped_event.payload["reason_code"] == "worker_lease_expired"
