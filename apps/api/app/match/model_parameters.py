...
class FrozenModelParametersError(ValueError):
    pass


@dataclass(frozen=True)
class FrozenPlayerModelConfiguration:
    provider: str
    model_id: str
...
def validate_frozen_model_parameters(
    ...
    *,
    ...
    if not isinstance(supports_thinking, bool):
        raise FrozenModelParametersError("model_supports_thinking must be a boolean")
    if not isinstance(values, dict):
        raise FrozenModelParametersError("model_parameters must be an object")
    if "reasoning_effort" not in values:
        raise FrozenModelParametersError("frozen reasoning_effort key is required")
    try:
...
def validate_players_snapshot_model_configurations(snapshot: object) -> None:
    ...
    for index, player in enumerate(snapshot):
        if not isinstance(player, dict):
            raise FrozenModelParametersError(f"players_snapshot[{index}] must be an object")
        try:
...
def frozen_player_model_configuration(
    player: object,
) -> FrozenPlayerModelConfiguration:
    if not isinstance(player, dict):
        raise FrozenModelParametersError("frozen player must be an object")
    provider = player.get("model_provider")
    model_id = player.get("model")
    if not isinstance(provider, str) or not provider.strip():
        raise FrozenModelParametersError("model_provider must be a non-empty string")
    if not isinstance(model_id, str) or not model_id.strip():
        raise FrozenModelParametersError("model must be a non-empty string")
    ...
    return FrozenPlayerModelConfiguration(
