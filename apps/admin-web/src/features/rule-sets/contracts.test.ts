...
describe("rule-set response contracts", () => {
  ...
  it.each([
    ...
    ["role maximum above player maximum", { ...options, roles: options.roles.map((role) => role.id === "villager" ? { ...role, max_count: 13 } : role) }],
    ["role minima exceed player maximum", { ...options, roles: options.roles.map((role) => role.id === "werewolf" ? { ...role, min_count: 5 } : role.id === "villager" ? { ...role, min_count: 11 } : role) }],
    ["role maxima cannot reach player minimum", { ...options, roles: options.roles.map((role) => ({ ...role, max_count: role.min_count })) }],
