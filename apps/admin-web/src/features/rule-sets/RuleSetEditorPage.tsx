...
function Editor({ options, initialDetail, isNew }: { options: RuleSetOptions; initialDetail?: AdminRuleSetDetail; isNew: boolean }) {
  ...
  return <div className="admin-page rule-set-editor-page"><header className="page-heading rule-set-heading"><div><span className="page-kicker">CONTENT / RULES</span><h1>{isNew ? "新建游戏规则" : "游戏规则详情"}</h1>{detail ? <p><span>{STATUS[detail.status]}</span>{detail.is_default ? " · 默认规则" : ""} · 规则锁版本 {detail.lock_version} · 草稿版本 {detail.draft_revision?.revision_no ?? "无"}</p> : <p>创建结构化规则草稿</p>}</div>{!editable ? <span className="page-readiness-badge">只读权限</span> : null}</header>
    ...
    <form className="rule-set-form" onSubmit={save}><fieldset disabled={!editable || pending !== null}><legend>结构化规则配置</legend>
      ...
      <Field label="规则说明" error={errors.description}><Input.TextArea aria-label="规则说明" onChange={(e) => changeConfig("description", e.target.value)} value={draft.config.description} /></Field>
      ...
      <fieldset className="rule-set-role-grid" aria-describedby={errors.role_counts ? "rule-error-role-counts" : undefined} aria-invalid={Boolean(errors.role_counts)}><legend>角色数量</legend>{options.roles.map((role) => <Field key={role.id} label={`${role.label}数量`}><InputNumber aria-describedby={errors.role_counts ? "rule-error-role-counts" : undefined} aria-invalid={Boolean(errors.role_counts)} aria-label={`${role.label}数量`} data-testid="role-count" max={role.max_count} min={role.min_count} onChange={(value) => changeConfig("role_counts", { ...draft.config.role_counts, [role.id]: value ?? Number.NaN })} value={Number.isFinite(draft.config.role_counts[role.id]) ? draft.config.role_counts[role.id] : null} /></Field>)}{errors.role_counts ? <p id="rule-error-role-counts" role="alert">{errors.role_counts}</p> : null}<p>总人数：{playerCount(draft)} 人</p><p>{roleSummary(draft, options) || "暂无角色"}</p></fieldset>
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) { const id = `rule-error-${label.replaceAll(" ", "-")}`; const control = isValidElement<Record<string, unknown>>(children) ? cloneElement(children, { "aria-invalid": error ? true : children.props["aria-invalid"] ?? false, "aria-describedby": error ? id : children.props["aria-describedby"], status: error ? "error" : children.props.status }) : children; return <label className="rule-set-field"><span>{label}</span>{control}{error ? <small id={id} role="alert">{error}</small> : null}</label>; }
...
function Choice({ label, value, choices, onChange, error }: { label: string; value: string; choices: { value: string; label: string }[]; onChange: (v: string) => void; error?: string }) { return <Field label={label} error={error}><Select aria-label={label} onChange={onChange} options={choices} value={value} /></Field>; }
function Check({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) { return <Checkbox aria-label={label} checked={checked} className="rule-set-check" onChange={(event) => onChange(event.target.checked)}>{label}</Checkbox>; }
