  ...
  return <div className="admin-page rule-set-page">
    ...
    <section aria-label="规则筛选" className="rule-set-filter-panel">
      ...
      <div className="rule-set-filter-grid">
        <label><span>生命周期</span><select onChange={(event) => updateSearch({ status: event.target.value })} value={params.status ?? ""}><option value="">全部状态</option>{options.statuses.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
        <label><span>玩家人数</span><select onChange={(event) => updateSearch({ player_count: event.target.value })} value={params.player_count ?? ""}><option value="">全部人数</option>{playerCounts(options.constraints.player_count_min, options.constraints.player_count_max).map((count) => <option key={count} value={count}>{count} 人</option>)}</select></label>
        <label><span>排序</span><select onChange={(event) => { const signed = event.target.value; updateSearch({ sort: signed.replace(/^-/, ""), direction: signed.startsWith("-") ? "desc" : "asc" }); }} value={`${params.direction === "desc" ? "-" : ""}${params.sort}`}>{options.sorts.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
