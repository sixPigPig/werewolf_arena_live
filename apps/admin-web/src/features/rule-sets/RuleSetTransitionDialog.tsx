...
export default function RuleSetTransitionDialog({ title, description, confirmLabel, pendingLabel, reasonMinLength, reasonMaxLength, pending, error, candidates = [], requiresReplacement = false, candidatesPending = false, candidatesError = false, opener, onCancel, onConfirm }: Props) {
  ...
  useEffect(() => { dialog.current?.querySelector<HTMLElement>('input:not([disabled]), textarea:not([disabled]), button:not([disabled])')?.focus(); return () => opener?.focus(); }, [opener]);
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLElement>) {
    ...
    const focusable = Array.from(dialog.current?.querySelectorAll<HTMLElement>('input:not([disabled]), textarea:not([disabled]), button:not([disabled])') ?? []); if (!focusable.length) return;
  ...
  return <div className="rule-set-dialog-backdrop"><section aria-describedby={descriptionId} aria-labelledby={titleId} aria-modal="true" className="rule-set-dialog" onKeyDown={handleKeyDown} ref={dialog} role="dialog">
    ...
    <form onSubmit={submit}>
      {requiresReplacement ? <label><span>替代默认规则</span><Select aria-label="替代默认规则" disabled={pending || candidatesPending || candidatesError || noCandidate} onChange={setReplacementId} optionFilterProp="label" options={[{ label: "请选择已发布规则", value: "" }, ...candidates.map((candidate) => ({ label: candidate.draft_revision?.config?.name ?? candidate.published_revision?.config?.name ?? candidate.id, value: candidate.id }))]} showSearch value={replacementId} /></label> : null}
      ...
      <label><span>操作原因</span><Input.TextArea aria-describedby={reasonError ? reasonErrorId : undefined} aria-invalid={Boolean(reasonError)} aria-label="操作原因" disabled={pending} maxLength={reasonMaxLength} minLength={reasonMinLength} onChange={(event) => { setReason(event.target.value); setReasonError(null); }} status={reasonError ? "error" : undefined} value={reason} /></label>
