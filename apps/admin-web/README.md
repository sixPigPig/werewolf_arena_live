...
当前后台已连接真实服务端认证、运营总览、任务中心、只读安全设置、全局 ID 搜索、玩家/对局/运行/语音/账号/审计模块。玩家管理支持服务端分页、草稿编辑、受控 AI 填表、发布、归档、恢复、权限、CSRF、乐观锁与结构化错误；AI 结果不会自动保存或发布。未接入 API 的规划模块不进入导航和路由。

生产构建只有显式设置 `VITE_ADMIN_AUTH_ENABLED=true` 才会连接认证链，否则 fail closed。固定角色、服务端权限依赖以及玩家创建、更新、AI 草稿、发布、归档、恢复、运行控制、语音任务和账号治理的成功/失败审计已经落地；正式身份源和部署安全验收完成前，不提供生产登录入口。
