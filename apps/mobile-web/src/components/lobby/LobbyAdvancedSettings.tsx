...
type LobbyAdvancedSettingsProps = {
  audioMode: V2ConfiguredAudioMode;
  disabled: boolean;
  maxRounds: string;
  maxRoundsError: string | null;
  onAudioModeChange: (value: V2ConfiguredAudioMode) => void;
  ...
  return (
    <details className="mobile-lobby-advanced" ref={detailsRef}>
      ...
      <div className="mobile-lobby-settings-grid">
        ...
        <label className="mobile-lobby-field"><span>播报方式</span><select disabled={disabled} onChange={(event) => onAudioModeChange(event.target.value as V2ConfiguredAudioMode)} value={audioMode}><option value="tts">语音播报</option><option value="text_only">纯文本（无 TTS）</option></select></label>
      </div>
